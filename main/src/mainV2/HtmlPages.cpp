    ...
    <script>
        ...
        const ui = {
            ...
            updateDashboard: (data) => {
                ...
                const elAutoToggle = document.getElementById("autoToggle");
                const elModeLabel = document.getElementById("modeLabel");

                if (!elTemp || !elHum || !elVoltage || !elCurrent || !elUnit || !elBadge) {
                    console.log("Dashboard DOM is not ready");
                    return;
                }
                ...
                elTemp.innerText = t;
                elHum.innerText = h;
                elVoltage.innerText = v;
                elCurrent.innerText = c;

                if (Number.isFinite(Number(data.high))) appState.settings.high = Number(data.high);
                if (Number.isFinite(Number(data.low))) appState.settings.low = Number(data.low);
                ...
                if (elAutoToggle) elAutoToggle.checked = isAuto;
                if (elModeLabel) elModeLabel.innerText = isAuto ? "AUTO" : "MAN";
